...
const SCREENS: Screen[] = [
  ...
  },
  {
    ...
    accentBg: 'rgba(57,217,138,0.1)',
    bars: [55, 68, 72, 80, 85, 91, 95],
  },
];

function MockupCard({ screen }: { screen: Screen; flip?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="h-7 w-7 rounded-xl flex items-center justify-center" style={{ background: `${color}20` }}>
      ...
      <div className="flex items-end gap-1.5 flex-1">
        {bars.map((h, i) => (
          <div key={i} className="flex-1 rounded-t-md transition-all" style={{ height: `${h}%`, background: i === bars.length - 1 ? color : `${color}40` }} />
        ))}
      </div>

      {/* Glow overlay */}
      <div className="absolute inset-0 rounded-3xl pointer-events-none" style={{ background: `radial-gradient(circle at 50% 80%, ${accentBg} 0%, transparent 70%)` }} />
    </div>
  );
}
