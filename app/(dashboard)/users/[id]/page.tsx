...
const BAR_COLORS = [
  'bg-primary', 'bg-accent', 'bg-emerald-400', 'bg-amber-400',
  'bg-sky-400', 'bg-rose-400', 'bg-violet-400', 'bg-teal-400',
];
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="h-1.5 rounded-full bg-white/5 overflow-hidden">
        <div
          className={cn('h-full rounded-full transition-all duration-500', BAR_COLORS[idx % BAR_COLORS.length])}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
    ...
    <div className="glass-card rounded-2xl p-4 space-y-4">
      ...
      {record.deviceInfo && (
        <p className="text-[10px] text-muted-foreground -mt-2">
          {record.deviceInfo.deviceName} · {record.deviceInfo.osVersion}
        </p>
      )}
      <div className="space-y-2.5">
        {sorted.length === 0 ? (
          <p className="text-xs text-muted-foreground">No app data recorded.</p>
        ) : (
          sorted.map((app, i) => (
            <AppBar key={app.packageName} app={app} max={max} idx={i} />
          ))
        )}
      </div>
    </div>
...
export default function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  records.forEach((r) =>
    r.appsUsage.forEach((a) => {
      appTotals[a.appName] = (appTotals[a.appName] ?? 0) + a.usageTime;
    })
  );
    ...
    <div className="space-y-8 w-full">
      ...
      <div className="flex items-center gap-3">
        <Link href="/users">
          <Button
            variant="ghost"
            size="icon"
            className="h-8 w-8 rounded-xl text-muted-foreground hover:text-foreground hover:bg-white/5"
          >
        ...
        <div className="flex-1 min-w-0">
          {userLoading ? (
            <>
              ...
              <Skeleton className="h-3.5 w-48 bg-white/5 mt-1.5" />
            </>
          ) : (
            <>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Badge
              ...
              className={cn(
                ...
                user.role === 'SUPER_ADMIN' ? 'bg-primary/15 text-primary' : 'bg-white/5 text-muted-foreground'
              )}
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard
          ...
          value={analyticsLoading ? '—' : fmtMins(totalTime)}
          sub={`${records.length} day${records.length !== 1 ? 's' : ''} tracked`}
          isLoading={analyticsLoading}
        />
        <KpiCard
          icon={TrendingUp}
          label="Daily Average"
          value={analyticsLoading ? '—' : fmtMins(avgTime)}
          sub="per day"
          isLoading={analyticsLoading}
        />
        <KpiCard
          icon={BarChart2}
          label="Top App"
          value={analyticsLoading ? '—' : (topApp?.[0] ?? 'N/A')}
          sub={topApp ? fmtMins(topApp[1]) + ' total' : undefined}
          isLoading={analyticsLoading}
        />
        <KpiCard
          icon={Smartphone}
          label="Last Login"
          value={userLoading ? '—' : (user?.lastLogin ? new Date(user.lastLogin).toLocaleDateString() : 'Never')}
          sub={user?.lastLogin ? formatDate(user.lastLogin) : undefined}
      ...
      <div className="space-y-4">
        ...
        {analyticsLoading ? (
            ...
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="glass-card rounded-2xl p-4 space-y-3">
                ...
                {Array.from({ length: 5 }).map((_, j) => (
                  <Skeleton key={j} className="h-5 w-full bg-white/5 rounded" />
                ))}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {records.map((record) => (
              <DayCard key={record._id} record={record} />
            ))}
          </div>
        )}
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="flex items-center justify-between px-1 pt-2">
            <p className="text-xs text-muted-foreground">
              Page {pagination.page} of {pagination.totalPages} &mdash; {pagination.total} records
            </p>
            <div className="flex gap-2">
              <Button
                ...
                className="h-8 rounded-xl border-border/60 bg-white/5 hover:bg-white/8 text-xs gap-1"
              >
